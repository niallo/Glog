...
Glog.prototype.load_configs = function(cb) {
	...
	var options = {
            ...
            show_author : false,
            // Heroku exports the port to listen on via UNIX environmet variable "PORT"
...
Glog.prototype.load_articles = function(options, cb) {
	...
	var articles = [],
		article_path = path.join('blog', '/articles'),
		fn = this;
...
Glog.prototype.render_blog = function(options, articles, cb) {
	...
	fs.readFile(path.join('blog', 'layout.jade'), 'UTF-8', function(err, data) {
		if(err) {
